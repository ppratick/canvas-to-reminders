...
# Ignore SSL warnings on macOS with LibreSSL
...
def generate_study_strategy(assignments, avg_days, top_day, urgent_task):
    ...
    all_tasks.sort(key=lambda x: datetime.strptime(x[2], "%m/%d/%Y %H:%M"))
...
for course in favorite_courses:
    ...
    for index, assignment in enumerate(assignments, start=1):
        percent = int((index / len(assignments)) * 100)
        print(f"\r🔄 Syncing {course_name}... [{index} / {len(assignments)}] ({percent}%)", end="", flush=True)
        ...
        try:
            ...
            display_due = local_due.strftime("%m/%d/%Y %H:%M")
    ...
    print()  # new line after each course
